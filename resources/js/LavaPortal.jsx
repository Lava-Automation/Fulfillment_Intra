...
function AccountsPage({ onOpenAcct }) {
  ...
  const filtered = useMemo(() => {
    let data = ACCOUNTS_DATA.filter(a => {
    ...
    return data;
  }, [search, fPm, fStage, fCs, fFulfill, fTix, fProg, sort]);
  ...
  return (
    <div className="page-enter">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:10, marginBottom:10 }}>
        <div><h2 style={{ ...fd, fontSize:28, fontWeight:800 }}>Accounts</h2><p style={{ ...fm, fontSize:11, color:T.ink3, marginTop:3 }}>{ACCOUNTS_DATA.length} active client accounts</p></div>
...
export default function App() {
  ...
  const handleLogin = () => {
    if (!email || !pass) { setLoginErr(true); return; }
    setLoginErr(false);
  ...
  if (!loggedIn) {
    return (
      <>
        ...
        <div style={{ position:'fixed', inset:0, display:'flex', alignItems:'center', justifyContent:'center', background:'#121210' }}>
          <div style={{ width:360, background:'#1C1B18', border:'1px solid #333', borderTop:`2px solid ${T.lava}`, padding:'40px 32px 32px' }}>
            <h1 style={{ ...fd, fontSize:36, fontWeight:800, letterSpacing:2, color:'#F0EDE8', marginBottom:2 }}>L<span style={{ color:T.lava }}>A</span>VA</h1>
            <p style={{ ...fm, fontSize:10, color:'#555', letterSpacing:2, marginBottom:32 }}>Internal Staff Portal</p>
            {loginErr && <div style={{ background:'rgba(217,58,12,.15)', border:`1px solid ${T.lava}`, color:T.lava2, padding:'9px 12px', ...fm, fontSize:12, marginBottom:14 }}>Please enter your email and password.</div>}
            <label style={{ display:'block', ...fm, fontSize:10, letterSpacing:2, color:'#666', textTransform:'uppercase', marginBottom:6 }}>Staff Email</label>
            <input type="text" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key==='Enter'&&handleLogin()} placeholder="you@lavaautomation.com" style={{ width:'100%', background:'#111', border:'1px solid #333', color:'#F0EDE8', padding:'11px 14px', ...fm, fontSize:13, marginBottom:18 }} />
            <label style={{ display:'block', ...fm, fontSize:10, letterSpacing:2, color:'#666', textTransform:'uppercase', marginBottom:6 }}>Password</label>
            <input type="password" value={pass} onChange={e => setPass(e.target.value)} onKeyDown={e => e.key==='Enter'&&handleLogin()} placeholder="Any password" style={{ width:'100%', background:'#111', border:'1px solid #333', color:'#F0EDE8', padding:'11px 14px', ...fm, fontSize:13, marginBottom:20 }} />
            <button onClick={handleLogin} style={{ width:'100%', background:T.lava, color:'#fff', border:'none', padding:13, ...fd, fontSize:17, fontWeight:700, letterSpacing:3, cursor:'pointer' }}>ACCESS DASHBOARD</button>
          </div>
        </div>
      </>
    );
  ...
  const topTitle = openAcctId ? (ACCOUNTS_DATA.find(a=>a.id===openAcctId)?.name || 'Account') : (PAGE_LABELS[page] || page);
      ...
      <div style={{ display:'flex', minHeight:'100vh' }}>
        ...
        <nav style={{ width:220, background:'#121210', color:'#999', display:'flex', flexDirection:'column', position:'sticky', top:0, height:'100vh', flexShrink:0, overflowY:'auto' }}>
          ...
          <div style={{ marginTop:'auto', padding:'14px 20px', borderTop:'1px solid #222', display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:30, height:30, background:T.lava, display:'flex', alignItems:'center', justifyContent:'center', ...fd, fontSize:13, fontWeight:700, color:'#fff' }}>{username.charAt(0)}</div>
            <div style={{ fontSize:12, color:'#CCC', flex:1 }}>{username}</div>
            <button onClick={() => setLoggedIn(false)} style={{ background:'none', border:'none', color:'#555', cursor:'pointer', fontSize:14 }}>⏻</button>
          </div>
        </nav>
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
          ...
          <div style={{ flex:1, padding:24 }}>
            {page === 'dashboard'   && !openAcctId && <Dashboard onNav={navTo} onOpenAcct={openAcct} />}
            {page === 'accounts'    && !openAcctId && <AccountsPage onOpenAcct={openAcct} />}
            {page === 'accounts'    && openAcctId  && <AccountDetail acctId={openAcctId} accounts={ACCOUNTS_DATA} onBack={() => { setOpenAcctId(null); setOpenAcctTab(null); }} initialTab={openAcctTab} />}
